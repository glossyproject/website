  ...
  navGroups: [
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Sales", icon: LineChart, href: "#" },
      ],
    },
  ],
  footerGroup: {
...
const segments: Segment[] = [
  { name: "Computers & tablets", value: 78000, color: palette.primary },
  { name: "Mobile", value: 52000, color: palette.secondary.light },
  { name: "Wearables & audio", value: 35000, color: palette.tertiary.light },
  { name: "Home & TV", value: 24541, color: palette.quaternary.light },
];
...
const categoryBreakdown: CategoryBreakdownRow[] = [
  ...
  { name: "Mobile", orderCount: 5340, changePercent: 19.1, isPositive: true },
  {
...
const revenueChartConfig = {
  thisYear: { label: "Gross sales", color: palette.primary },
  prevYear: { label: "Prior year", theme: palette.secondary },
...
const SegmentationPanel = () => {
  ...
  const totalSegmentSales = segments.reduce((sum, s) => sum + s.value, 0);
      ...
      <div className="flex flex-col gap-4 p-4 sm:p-5">
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-semibold sm:text-3xl">
            {compactCurrencyFormatter.format(totalSegmentSales)}
          </span>
          <span className="text-sm text-muted-foreground">MTD gross sales</span>
        </div>
              ...
              <ShadTooltip>
                <ShadTooltipTrigger asChild>
                  <button
                    ...
                    aria-label={`${segment.name}: ${currencyFormatter.format(segment.value)} (${Math.round((segment.value / totalSegmentSales) * 100)}%)`}
                  />
                </ShadTooltipTrigger>
                <ShadTooltipContent
                  side="top"
                  ...
                  <div className="grid gap-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground tabular-nums">
                        {Math.round((segment.value / totalSegmentSales) * 100)}%
                      </span>
                    </div>
                    <span className="text-muted-foreground tabular-nums">
...
const RevenueFlowChart = () => {
  ...
  const totalGrossSales = chartData.reduce(
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-4">
        <div className="flex flex-1 flex-col gap-1">
          <p className="text-xl leading-tight font-semibold tracking-tight sm:text-2xl">
            {currencyFormatter.format(totalGrossSales)}
          </p>
          <p className="text-xs text-muted-foreground">
            Gross sales ({periodLabels[period].toLowerCase()})
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:gap-4 sm:px-6 sm:py-3.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 sm:flex-none">
            ...
            />
            <Input
              placeholder="Search orders, customer, category…"
              value={searchQuery}
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              ...
              <TableHead className="hidden min-w-[128px] text-xs font-medium text-muted-foreground sm:table-cell sm:text-sm">
                Category
              </TableHead>
            ...
            {paginatedOrders.length === 0 ? (
              ...
              paginatedOrders.map((order, index) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell className="hidden max-w-[160px] truncate text-xs text-muted-foreground sm:table-cell sm:text-sm">
                    {order.category}
                  </TableCell>
