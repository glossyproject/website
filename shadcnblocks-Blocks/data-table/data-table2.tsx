...
export const DataTableColumnHeader = <TData, TValue>({
  ...
  if (!canSort) {
    return (
      <span className="flex h-8 items-center px-2 text-sm font-medium text-foreground">
        {title}
      </span>
    );
  }
        ...
        <div className="w-full">
          <div className="mb-8 text-left">
            ...
            <p className="mt-2 text-muted-foreground">
              Enhanced table with clear visual borders and separation between
              cells. Ideal for detailed data visualization where each cell needs
              distinct boundaries.
            </p>
          </div>
          <div className="overflow-hidden rounded-lg border-1 border-border">
                  ...
                  <TableRow key={headerGroup.id}>
                    {headerGroup.headers.map((header) => {
                      return (
                        <TableHead
                          key={header.id}
                          className="border-r px-0 last:border-r-0"
                        >
