...
interface Hero276Props extends HeroFeatureSliderProps {}
type Props = Partial<Hero276Props>;

const defaultProps: Hero276Props = {
...
const Hero276 = (props: Props) => {
  const { heading, features, images, className } = {
    ...defaultProps,
    ...props,
  };
    ...
    <section className={cn("py-32", className)}>
      <div className="container mx-auto overflow-hidden">
        <div className="mb-14 text-center lg:mb-16">
          <h1 className="mx-auto max-w-3xl text-5xl font-semibold tracking-tight text-pretty lg:text-6xl">
            {heading}
          </h1>
        </div>

        <div
          className="mb-10 flex w-full flex-col md:flex-row"
            ...
            <React.Fragment key={feature.title}>
              {index > 0 && (
                <Separator
                  ...
                  className="mx-4 hidden h-auto w-0.5 shrink-0 bg-linear-to-b from-muted via-transparent to-muted md:mx-6 md:block"
                />
              )}
              <div
                className="flex grow basis-0 cursor-default flex-col items-center rounded-xl p-4 text-center transition-colors hover:bg-muted/40 md:items-start md:text-left"
            ...
            {visibleFeatures.map((_, i) => (
              <span
                ...
                className={cn(
                  ...
                  i === activeImageIndex ? "bg-primary" : "bg-muted",
                )}
              />
            ))}
          </div>
